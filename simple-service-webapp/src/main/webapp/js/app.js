...
filterApp.factory('PropertyGroups', [ '$resource', function($resource) {
	return $resource('api/v1/propertygroups/:id', {
		id : '@id'
	}, {
		'update' : {
			method : 'PUT'
...
filterApp.factory('PropertyGroupsConnection', [ '$resource', function($resource) {
	return $resource('api/v1/propertygroups/:id/connection', {
		id : '@id'
	}, {
		'disconnect' : {
			method : 'DELETE'
...
filterApp.factory('PropertyConnection', [ '$resource', function($resource) {
	return $resource('api/v1/property/:id/connection/:pGroupId', {
		id : '@id',
		pGroupId : '@pGroupId'
	}, {
		'connect' : {
...
filterApp.factory('AttributesByGroup', [ '$resource', function($resource) {
	return $resource('api/v1/property/bygroup/:groupId', {
		groupId : '@id'
	});
} ]);

filterApp.controller('MainController', [ '$scope', '$routeParams', 'PropertyGroups', function($scope, $routeParams, PropertyGroups) {
	...
	$scope.updatePropertyGroup = function(index, document) {
		...
		PropertyGroups.update({
			id : group.id
		}, group, function(value, responseHeaders) {
...
filterApp
		.controller(
				...
				[
						...
						'AttributesByGroup',
						'PropertyGroups',
						'PropertyGroupsConnection',
						'PropertyConnection',
						function($scope, $routeParams, Attributes, AttributesByGroup, PropertyGroups, PropertyGroupsConnection,
								PropertyConnection) {
							$scope.dataLoading = true;
							...
							$scope.propertyGroupsLinkedFilters = PropertyGroups.query({
								excludeConnectionType : "CHILD",
								type : "dropdown"
							}, function(value, responseHeaders) {
								$scope.dataLoading = false;
							...
							$scope.dataLoading = true;
							$scope.freePropertyGroups = PropertyGroups.query({
								includeConnectionType : "NOT_CONNECTED"
							}, function(value, responseHeaders) {
								$scope.dataLoading = false;
							}, function(httpResponse) {
								$scope.dataLoading = false;
							...
							$scope.getCss = function(ngModelController) {
								var resultCss = ngModelController.$valid ? "ng-valid" : (ngModelController.$pristine ? "ng-valid"
										: "invalidControl");
								return resultCss;
							};
							$scope.getButtonCss = function(disabled) {
							...
							$scope.myForm.updateProperty = function(index, document) {
								...
								Attributes.update({
									id : attribute.id
								}, attribute, function(value, responseHeaders) {
									attribute.processing = false;
								}, function(httpResponse) {
									attribute.processing = false;
							...
							// 2. Then
							// Utils.createAttributesAndDraggableChildFilters
							// displays them & attaches drag & drop events
							// 3. The drop event on a parent attribute (given
							// filters attribute) calls $scope.disconnect
							// 4. Scope.disconnect() recompute the free filters
							// (property group) and calls a custom "callback"
							// function (for disconnect it is
							// "loadParentFilterAttributes('filterAttributes')",
							// specified from the linked_filters.view.html
							//	
							$scope.loadParentFilterAttributes = function(uiControlId) {
								...
								$scope.attributes = AttributesByGroup.query({
									groupId : selectedPropertyGroup
								}, function(value, responseHeaders) {
									$scope.dataLoading = false;
							...
							$scope.disconnect = function(filterToDisconnect, callback) {
								$scope.dataLoading = true;
								PropertyGroupsConnection.disconnect({
									id : filterToDisconnect
								}, function(value, responseHeaders) {
							...
							$scope.connect = function(child, parentAttribute) {
								...
								PropertyConnection.connect({
									...
									$scope.dataLoading = false;
									if ("occupied" == httpResponse.headers("x-additionalparam")) {
										alert("Този атрибут вече е свързан.");
									} else {
...
filterApp.controller('AddLocationController', [ '$scope', '$http', 'PropertyGroups', 'Locations',
		function($scope, $http, PropertyGroups, Locations) {
			$scope.propertyGroups = PropertyGroups.query();
			$scope.s = "-1";
...
filterApp.controller('UpdateLocationController', [
		'$scope',
		'$http',
		'$routeParams',
		'PropertyGroups',
		...
		function($scope, $http, $routeParams, PropertyGroups, Locations) {
			...
			$scope.showHideChild = function(childPropertyGroupId, isChild, uiControlId, locationId, editMode, fields,
					disableFieldsIfinstructed) {

				Utils.showHideChildBase(childPropertyGroupId, isChild, uiControlId, $scope, $http, locationId, editMode, fields,
						disableFieldsIfinstructed);
			};
			$scope.s = "-1";
